...
# sed -i 's/^command=x11vnc.*/command=su - ubuntu -c "x11vnc -xkb -noxrecord -noxfixes -noxdamage -display :1 -rfbauth \/tmp\/.password2 -shared -forever"/' /etc/supervisor/conf.d/supervisord.conf
...
# autorestart=true
# user=ubuntu
# stderr_logfile=/var/log/supervisor/agent_server.err.log
# stdout_logfile=/var/log/supervisor/agent_server.out.log
